...
def check_single_page(page_data):
    ...
    for scale in np.linspace(0.5, 1.5, 10):
        ...
        if (
            height == 0
            or width == 0
            or height > img_array.shape[0]
            or width > img_array.shape[1]
        ):
            continue
...
def find_page_parallel(pdf_path, template_b64, total_pages, threshold=0.7):
    ...
    # מומלץ להתחיל עם 1–2 תהליכים; בשרתים חינמיים יותר מזה יכול לקרוס
...
def main():
    ...
    pdf_content = st.session_state.get("pdf_content", None)
    ...
    # טאאב לינק גוגל דרייב
    with tab2:
        gdrive_url = st.text_input(
            "הדבק כאן לינק לשיתוף מגוגל דרייב:",
            placeholder="https://drive.google.com/file/d/...",
        )
        if gdrive_url:
            if st.button("טען קובץ מהלינק"):
                with st.spinner("מוריד קובץ מגוגל דרייב..."):
                    gdrive_content = download_from_gdrive(gdrive_url)
                    if gdrive_content:
                        pdf_content = gdrive_content
                        st.success("הקובץ נטען בהצלחה!")
                    else:
                        st.error(
                            "לא ניתן להוריד את הקובץ. וודא שהלינק פתוח לצפייה לכולם (Anyone with the link)."
                        )

    # שמירת התוכן ב-session_state
    if pdf_content:
            ...
            try:
                ...
                if extract_pdf_by_images(temp_input_path, temp_output_path, start_b64, end_b64):
                    ...
                    with open(temp_output_path, "rb") as f:
                        st.download_button(
                            "📥 הורד קובץ חתוך", f, "output.pdf", "application/pdf"
                        )
                else:
